...
**System Overview**: The system Minchoom has 4 main components: Video player, Lecture Timeline, MinChat (general chat) and Catch Up mode. Below we will explain newly introduced features.

**Lecture Timeline**: Flags indicate the starting points of each section. The above buttons are clicked by the listeners, aggregated to generate the flags on the lecture timeline.

**Catch Up Mode**: Instead of general chat (MinChat), people can use Catch Up mode to ask with context and get timely answers. Questions can be asked by clicking on the flags in the lecture timeline, and answers are upvoted/downvoted.

**Assigned question**: Unanswered questions will be randomly assigned to the listers and pop out after a certain time limit. Listeners can choose to reject. The questions will popup in the general chat (MinChat).

### Instructions
The [following](https://www.figma.com/proto/JHYZayKschscpM5sQty0GX/CS473-DP3-Low-fi?node-id=133:643&scaling=min-zoom) is a low-fi prototype of a system that supports distracted users in online lectures to quickly catch up with the context. The system is composed of four features: (1) the main lecture screen where the real-time lecture is being held, (2) the lecture timeline generated by the listeners, (3) MinChat (general chat), and (4) the Catch Up mode where listeners ask and answer questions.
