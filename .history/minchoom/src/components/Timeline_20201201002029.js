...
export default class Timeline extends React.Component {
    ...
    showPreview = () => {
        this.setState({hoverPreview: true});
    }
            ...
            <div className="progressBar-container">
                <div className="progressBar">
                    ...
                    <div onMouseOut = {() => this.setState({hoverPreview: false})}>
                        <div className="preview" hidden={hoverPreview != true} onMouseOver = {showPreview} onMouseOut = {() => this.setState({hoverPreview: false})} style={{left: value[1]/videoTime*65+"%"}}>
                            <img   className={this.state.hoverPreview ? 'questionImg' : 'flag-loading-hidden'} src={flagToImg(value[1])}  />
                            <div className={this.state.hoverPreview ? null : 'flag-loading-hidden'}style={{color: 'grey'}}>Flagged at {formatTime(value[1])}</div>
                        </div>
                        <Clickable onClick={() => flagClickHandler(value)}>
                            <div className="flag-tip" key={value[1]} style={{left: value[1]/videoTime*65+"%"}} onMouseOver={showPreview}>
                                { value[0] === "Activity" 
                                    ?
                                    '📝'
                                    :
